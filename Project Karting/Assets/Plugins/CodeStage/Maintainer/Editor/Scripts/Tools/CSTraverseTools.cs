		...
		public static void TraverseMaterialTexEnvs(SerializedObjectTraverseInfo traverseInfo, MaterialSavedPropertyTraverseCallback callback)
		{
			...
			if (texEnvs != null)
			{
				for (var k = 0; k < texEnvs.arraySize; k++)
				{
					...
					{
						Debug.LogError(Maintainer.ConstructError("Can't get second.m_Texture from texEnvs at " + traverseInfo.TraverseTarget.name));
					}
			...
			{
				Debug.LogError(Maintainer.ConstructError("Can't get m_SavedProperties.m_TexEnvs.Array from material instance at " + traverseInfo.TraverseTarget.name));
			}
		}
