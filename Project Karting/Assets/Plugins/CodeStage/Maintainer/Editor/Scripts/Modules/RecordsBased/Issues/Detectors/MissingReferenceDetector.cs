...
namespace CodeStage.Maintainer.Issues.Detectors
{
	...
	internal class MissingReferenceDetector : IssueDetectorBase
	{
		...
		public bool TryDetectSerializedPropertyIssue(string assetPath, string typeName, SerializedProperty property, string propertyNameOverride = null)
		{
			return TryDetectIssue(RecordLocation.Asset, assetPath, null, null, typeName, -1, property, propertyNameOverride);
		}

		public bool TryDetectIssue(RecordLocation location, string assetPath, GameObject target, Type componentType, string componentName, int orderIndex, SerializedProperty property, string propertyNameOverride = null)
		{
			if (!enabled || !IsPropertyHasMissingReference(property)) return false;

			AddIssue(location, assetPath, target, componentType, componentName, orderIndex, propertyNameOverride ?? property.propertyPath);
			return true;
		}

		public void TryDetectUnityObjectUnityEventIssue(string assetPath, string typeName, SerializedProperty property)
		{
		...
		private void AddIssue(RecordLocation location, string assetPath, GameObject target, Type componentType, string componentName, int orderIndex, string propertyPath)
		{
			...
			{
				record = UnityObjectAssetIssueRecord.Create(IssueKind.MissingReference, assetPath, componentName, propertyPath);
			}
