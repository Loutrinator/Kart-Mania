		...
		private static void ProcessAsset(AssetInfo asset, string assetName, int assetIndex, int totalAssets)
		{
			...
			else
			{
				if (assetType == null)
				{
					...
					{
						IssuesDetector.ProcessUnityObjectAsset(asset, null);
					}
				}
				...
				{
					var target = AssetDatabase.LoadAssetAtPath<Material>(asset.Path);
					IssuesDetector.ProcessMaterial(asset, target);
				}
				else if (assetType == CSReflectionTools.monoScriptType)
				{
					var target = AssetDatabase.LoadAssetAtPath<MonoScript>(asset.Path);
					IssuesDetector.ProcessMonoScript(asset, target);
				}
				else // for the rest supported asset types 
				{
					var unityObject = AssetDatabase.LoadMainAssetAtPath(asset.Path);
					IssuesDetector.ProcessUnityObjectAsset(asset, unityObject);
				}
			}
		}
