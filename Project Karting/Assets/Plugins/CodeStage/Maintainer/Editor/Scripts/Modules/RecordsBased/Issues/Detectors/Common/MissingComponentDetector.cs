...
namespace CodeStage.Maintainer.Issues.Detectors
{
	...
	internal class MissingComponentDetector : IssueDetectorBase
	{
		...
		public bool TryDetectUnityObjectAssetIssue(string assetPath, Object target)
		{
			if (target != null) return false;

			if (enabled)
			{
				var record = UnityObjectAssetIssueRecord.Create(IssueKind.MissingComponent, assetPath);
				issues.Add(record);
			}
